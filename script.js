...
// Create floating particles - Chain Summit style
...
// Wait for DOM to be fully loaded before initializing text rotation and particles
document.addEventListener("DOMContentLoaded", function() {
  ...
  function wrapTextInSpans(element) {
    ...
    element.innerHTML = text
      ...
      .map(
        (char, i) =>
          `<span class="char" style="animation-delay: ${i * 0.05}s">${char === " "
            ? "&nbsp;"
            : char}</span>`
      )
      .join("");
  ...
  // Initialize first text set only if textSets exist
